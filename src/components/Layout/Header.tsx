...
export default function Header({ onNavigate, onToggleMobileMenu, mobileMenuOpen }: HeaderProps) {
  ...
  return (
    <header className="h-14 bg-socc-surface/90 backdrop-blur-md border-b border-socc-border/20 shadow-[0_1px_3px_rgba(0,0,0,0.2)] flex items-center justify-between px-3 sm:px-4 z-20">
      {/* Left */}
      <div className="flex items-center gap-2 sm:gap-3">
        <button
          onClick={onToggleMobileMenu}
          className="p-2.5 -ml-1 rounded-lg hover:bg-socc-hover/80 transition-all duration-200 text-gray-400 hover:text-gray-200 md:hidden"
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <button
          ...
          className="relative p-2.5 rounded-lg hover:bg-socc-hover/80 transition-all duration-200 text-gray-400 hover:text-gray-200"
          title={notifCount > 0 ? `${notifCount} unreviewed items` : 'No new alerts'}
        >
          <Bell className="w-4 h-4" />
          {notifCount > 0 && (
            <span className="absolute top-1 right-1 min-w-[16px] h-4 flex items-center justify-center bg-red-500 text-white text-[9px] font-bold rounded-full px-1 ring-2 ring-socc-surface">
              {notifCount}
            </span>
          )}
        </button>

        {/* User info — shown when auth is enabled */}
        {authEnabled && user && (
          <div className="flex items-center gap-2 ml-1 pl-2 border-l border-socc-border/30">
            ...
            <button
              onClick={logout}
              className="p-2.5 rounded-lg hover:bg-red-500/10 transition-all duration-200 text-gray-500 hover:text-red-400"
              title="Sign out"
            >
